...
app.get("/", async (c) => {
  ...
  return c.html(gameLayout("Être vs Avoir", `
...
app.post("/answer", async (c) => {
  ...
  const item = auxiliaryItems.find(a => a.id === itemId);
  if (!item) return c.redirect("/french/auxiliary");

  const correct = answer === item.correct;
  const db = getDb(c.env.DB);
  await recordAnswer(db, user.id, "auxiliary", itemId, correct);
  ...
  return c.html(gameLayout("Être vs Avoir", `
    <div class="feedback ${correct ? "correct" : "incorrect"}">
      ...
      <div class="actions">
        <a href="/french/auxiliary?index=${index + 1}" class="btn btn-primary">Next Question →</a>
      </div>
    </div>
  `, user));
});
